...
test('permitted prediction paths hydrate a selected fixture when its context is missing', () => {
  assert.equal(shouldHydratePredictionContext({ role: 'guest' }, null, '123'), true);
  assert.equal(shouldHydratePredictionContext({ role: 'guest' }, { matchId: '123' }, '123'), false);
  assert.equal(shouldHydratePredictionContext({ role: 'user' }, null, '123'), true);
  assert.equal(shouldHydratePredictionContext({ role: 'admin' }, null, '123'), false);
  assert.equal(shouldHydratePredictionContext({ role: 'guest' }, null, ''), false);
});
